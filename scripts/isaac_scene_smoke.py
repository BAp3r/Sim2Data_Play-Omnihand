...
def render_scene(args, result):
    ...
    for name, (product, annotator) in cams.items():
        ...
        camera = UsdGeom.Camera(stage.GetPrimAtPath(CAMERAS[name]))
        ...
        images[name] = fn; stats[name] = {"shape": list(rgb.shape), "std": float(rgb.std()), "frame_valid": bool(rgb.shape == (480,640,3)), "scene_content_detected": bool(rgb.std() > 2), "K": k, "T_world_usd": np.asarray(UsdGeom.Xformable(stage.GetPrimAtPath(CAMERAS[name])).ComputeLocalToWorldTransform(0)).T.tolist()}
