...
本文件记录 2026-09-22 用户确认的目标。当前是 M0/M1 前置阶段，未验收物理抓取或正式机器人数据集。**设计、计划和 AGENTS.md 已交付；本轮补做本地离线检查以完善它们。此前暂停的资产深度审计、SDK 小样和 GPU 验证未自动恢复。**
...
测试后置指移到本轮文档完成后的实施轮次；实施时仍按依赖关卡验证，不把文档完成与物理验收混为一项：

1. **设计冻结（本轮）**：固定桌面中转任务、三路 RGB、坐标/单位、模块接口、仓库存储边界和模型分工。未知实测量留空，形成待补输入清单。
...
第一波最多三个 Luna Max 子代理，各自 worktree；第二波可先做 E 的 plan-only 框架，A/B 通过后再启动 C。当前本地复核只针对已合入基线，未完成模块留待各自阶段。公共 schema、依赖选择和最终集成由主会话负责。Sol High/Astra High 留给明确需要的专项复核，不额外制造并发写入。

## 代理分工与本轮收口

主会话负责视频/图像理解、设计、计划、公共 schema 和最终审阅；需要专项多模态复核时用 Astra High。Luna Max 仅接收已经拆清楚的小任务，Sol High 按需负责复杂代码适配/复核。最多三个子代理同时工作，不递归派生。

本轮 A/B/D 已实际启动过，B 的坐标工具与参数清单已合入本地开发分支；E 启动了计划器工作。用户要求文档优先后，A/D/E 已中断并保留各自 worktree 中的在途内容，未作为已完成成果合入。C 尚未启动。恢复实现时先盘点这些 worktree，不能重新重复下载或覆盖已有改动。

下一次推进先处理第 2 步的资产与尺寸输入；测试留到后续实施轮次并按依赖关卡执行。本轮不承诺物理抓取成功、SDK 兼容性或数据吞吐。
...
下一批本地可选工作是候选 URDF 的 XML/关节图/mesh 引用静态审计，以及已完成 plan-only 模块的 seed/划分/恢复测试；先审阅现有 worktree，再决定最小实现范围。官方 SDK 真正回读另列为 D，GPU 物理/相机验收另列为 M1–M4。
